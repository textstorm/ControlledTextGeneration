...
class BaseModel(object):
  def __init__(self, args, mode, iterator, vocab_table, reverse_vocab_table=None, scope=None, name=None):
  ...
  def build_encoder(self):
    ...
    with tf.variable_scope("encoder") as scope:
      ...
      if self.encoder_type == "uni":
        ...
        encoder_output, encoder_state = tf.nn.dynamic_rnn(cell=encoder_cell, inputs=encoder_embed_inp, 
            dtype=tf.float32, sequence_length=source_sequence_length, swap_memory=True)
        ...
        bw_cell = self._build_encoder_cell(self.hidden_size, 
            self.forget_bias, num_bi_layers, self.mode, self.initializer, self.dropout)
        bi_outputs, bi_state = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell, cell_bw=bw_cell, 
          inputs=encoder_embed_inp, dtype=tf.float32, sequence_length=source_sequence_length, 
          swap_memory=True)
  ...
  def _single_cell(self, num_units, forget_bias, mode, initializer, dropout):
    ...
    cell = tf.contrib.rnn.LSTMCell(num_units=num_units, forget_bias=forget_bias,
        initializer=initializer, state_is_tuple=True)
    if dropout > 0.0:
  ...
  def _build_embedding(self, vocab_size, embed_size, name):
    ...
    with tf.variable_scope("embedding") as scope:
      embedding = self._weight_variable([vocab_size, embed_size], name=name, initializer=initializer)
    return embedding
...
class VRAE(BaseModel):
  def __init__(self, args, mode, iterator, vocab_table, reverse_vocab_table=None, scope=None, name=None):
    super(VRAE, self).__init__(args=args, mode=mode, iterator=iterator, vocab_table=vocab_table,
        reverse_vocab_table=reverse_vocab_table, scope=scope, name=name)
  ...
  def build_graph(self):
    ...
    with tf.variable_scope("model"):
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        z = tf.random_normal([3, self.latent_size])
        logits, sample_id, final_state, = self.build_decoder(z)
  ...
  def build_decoder(self, z):
    ...
    z = tf.layers.dense(z, self.hidden_size * 2 * self.num_layers, kernel_initializer=self.initializer)
    z = tf.reshape(z, [-1, self.num_layers, 2, self.hidden_size])
    with tf.variable_scope("decoder") as decoder_scope:
      decoder_cell, decoder_initial_state = self._build_decoder_cell(self.hidden_size, self.forget_bias, 
          self.num_layers, z, self.mode, self.initializer, self.dropout)
      ...
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        helper = tf.contrib.seq2seq.TrainingHelper(inputs=decoder_embed_inp, 
            sequence_length=target_sequence_length, name="decoder_helper")

        my_decoder = tf.contrib.seq2seq.BasicDecoder(cell=decoder_cell, 
            helper=helper, initial_state=decoder_initial_state)
        ...
        if self.beam_width > 0:
          my_decoder = tf.contrib.seq2seq.BeamSearchDecoder(cell=decoder_cell, embedding=decoder_embed, 
              start_tokens=start_tokens, end_token=end_token, initial_state=decoder_initial_state, 
              beam_width=self.beam_width, output_layer=self.output_layer)
        else:
          helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(embedding=decoder_embed, 
              start_tokens=start_tokens, end_token=end_token)
          my_decoder = tf.contrib.seq2seq.BasicDecoder(cell=decoder_cell, helper=helper,
        ...
        output, final_state, _ = tf.contrib.seq2seq.dynamic_decode(my_decoder, 
            maximum_iterations=maximum_iterations, swap_memory=True, scope=decoder_scope)
  ...
  def build_loss(self, logits, mu, logvar):
    ...
    avg_loss = avg_rec_loss + avg_kl_loss * self.kl_weight * 0.1
    return avg_rec_loss, rec_ppl, avg_kl_loss, avg_loss

  def train(self, sess, kl_weight):
    ...
    return sess.run([self.update, 
                     ...
                     self.batch_size,
                     self.mu,
                     self.logvar], feed_dict={self.kl_weight: kl_weight})
...
class CVRAE(VRAE):
  def __init__(self, args, mode, iterator, vocab_table, reverse_vocab_table=None, scope=None, name=None):
    ...
    self.ra_embed_size = args.ra_embed_size
    
    super(CVRAE, self).__init__(args=args, mode=mode, iterator=iterator, vocab_table=vocab_table,
        reverse_vocab_table=reverse_vocab_table, scope=scope, name=name)
  ...
  def build_graph(self):
    ...
    with tf.variable_scope("model"):
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        encoder_state_with_att = tf.concat([encoder_state, condition], 1)
        mu, logvar = self.build_latent(encoder_state_with_att)
        self.mu, self.logvar = mu, logvar
        z = self.sample_z(mu, logvar)
        z_with_condition = tf.concat([z, condition], 1)
        logits, sample_id, final_state, = self.build_decoder(z_with_condition)
        rec_loss, rec_ppl, kl_loss, loss = self.build_loss(logits, mu, logvar)
      else:
        z = tf.random_normal([3, self.latent_size])
        condition = self.build_condition()
  ...
  def build_condition(self):
    ...
    with tf.variable_scope("condition"):
      ra_embedding = self._build_embedding(self.rating_size, self.ra_embed_size, "ra_embedding")
      ra_embed_inp = tf.nn.embedding_lookup(ra_embedding, iterator.rating)
      return tf.layers.dense(ra_embed_inp, 16, activation=tf.nn.softplus)
